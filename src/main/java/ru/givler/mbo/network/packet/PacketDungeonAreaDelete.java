...
public final class PacketDungeonAreaDelete implements IMessage {
  ...
  public PacketDungeonAreaDelete(String areaId) {
    this.areaId = areaId;
  }
  ...
  public void fromBytes(ByteBuf buf) {
    ...
    areaId = buf.readBytes(n).toString(io.netty.util.CharsetUtil.UTF_8);
  }
  ...
  public static final class Handler implements IMessageHandler<PacketDungeonAreaDelete, IMessage> {
    @Override
    public IMessage onMessage(final PacketDungeonAreaDelete message, MessageContext context) {
      ...
      EditorPacketAccess.schedule(
          ...
          new Runnable() {
            @Override
            public void run() {
              ItemStack held = EditorPacketAccess.heldEditor(player);
              if (held == null || held.getItem() != ItemRegistry.DungeonEditor) return;
              DungeonAreaSavedData data = DungeonAreaSavedData.get(player.worldObj);
              DungeonAreaRecord area = data.byId(message.areaId);
              if (area != null && area.isNear(player, 64D)) {
                area.deleteBlocks(player.worldObj);
                data.remove(player.worldObj, area);
                player.addChatMessage(new ChatComponentTranslation("mbo.dungeon.area.deleted"));
              }
            }
          });
      return null;
