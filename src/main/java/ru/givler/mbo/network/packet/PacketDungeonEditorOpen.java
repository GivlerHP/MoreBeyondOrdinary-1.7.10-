...
public final class PacketDungeonEditorOpen implements IMessage {
  ...
  private int type, restoreMode, restoreSeconds = 10;

  public PacketDungeonEditorOpen() {}

  public PacketDungeonEditorOpen(String areaId, int type, int restoreMode, int restoreSeconds) {
  ...
  public void fromBytes(ByteBuf buf) {
    ...
    type = buf.readByte();
    restoreMode = buf.readByte();
    restoreSeconds = buf.readInt();
  }
  ...
  public void toBytes(ByteBuf buf) {
    ...
    buf.writeBytes(v);
    buf.writeByte(type);
    buf.writeByte(restoreMode);
    buf.writeInt(restoreSeconds);
  }

  public static final class Handler implements IMessageHandler<PacketDungeonEditorOpen, IMessage> {
    @Override
    public IMessage onMessage(final PacketDungeonEditorOpen message, MessageContext context) {
          ...
          new Runnable() {
            ...
            public void run() {
              Minecraft.getMinecraft()
                  .displayGuiScreen(
                      new GuiDungeonEditor(
                          message.areaId,
                          message.type,
                          message.restoreMode,
                          message.restoreSeconds));
            }
          });
      return null;
    }
