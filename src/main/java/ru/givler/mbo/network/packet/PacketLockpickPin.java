...
public class PacketLockpickPin implements IMessage {
  ...
  public PacketLockpickPin(int x, int y, int z, int pin, int order) {
    this.x = x;
    this.y = y;
    this.z = z;
    this.pin = pin;
  ...
  public static class Handler implements IMessageHandler<PacketLockpickPin, PacketLockpickResult> {
    ...
    public PacketLockpickResult onMessage(PacketLockpickPin m, MessageContext ctx) {
      ...
      if (tile == null
          ...
          || !LockableAccess.hasLockpick(player)
          || player.getDistanceSq(m.x + .5, m.y + .5, m.z + .5) > 64.0)
        return new PacketLockpickResult(false, m.pin, true, false, true);
      ...
      player.worldObj.playSoundEffect(
          ...
          m.y + .5,
          m.z + .5,
          correct ? "mbo:lock_pin_match" : "mbo:lock_pin_fail",
          1F,
          .9F + player.getRNG().nextFloat() * .2F);
      ...
      if (broke) {
        ...
        player.worldObj.playSoundAtEntity(
            player, "random.break", .8F, .85F + player.getRNG().nextFloat() * .2F);
      }
