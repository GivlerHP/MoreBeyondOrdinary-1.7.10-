...
public class PacketPlatformAction implements IMessage {
    ...
    public static class Handler implements IMessageHandler<PacketPlatformAction,IMessage>{
        @Override public IMessage onMessage(PacketPlatformAction m,MessageContext ctx){
            ...
            else if(m.action==GET_A||m.action==GET_B){net.minecraft.item.ItemStack stack=new net.minecraft.item.ItemStack(ru.givler.mbo.registry.BlockRegistry.PlatformStation);stack.setTagCompound(new net.minecraft.nbt.NBTTagCompound());net.minecraft.nbt.NBTTagCompound tag=stack.getTagCompound();tag.setString("PlatformId",p.getPlatformId().toString());tag.setBoolean("TargetB",m.action==GET_B);tag.setInteger("Dimension",player.dimension);tag.setInteger("AX",p.getEndpointAX());tag.setInteger("AY",p.getEndpointAY());tag.setInteger("AZ",p.getEndpointAZ());tag.setInteger("BX",p.getEndpointBX());tag.setInteger("BY",p.getEndpointBY());tag.setInteger("BZ",p.getEndpointBZ());if(!player.inventory.addItemStackToInventory(stack))player.dropPlayerItemWithRandomChoice(stack,false);}
