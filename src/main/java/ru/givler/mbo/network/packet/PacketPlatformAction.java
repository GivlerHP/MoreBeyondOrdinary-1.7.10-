  ...
  public static class Handler implements IMessageHandler<PacketPlatformAction, IMessage> {
    ...
    private void apply(PacketPlatformAction m,EntityPlayerMP player){
      ...
      if (m.action == SAVE) {
        ...
        tag.setInteger("AZ", p.getEndpointAZ());
        tag.setInteger("BX", p.getEndpointBX());
        tag.setInteger("BY", p.getEndpointBY());
        tag.setInteger("BZ", p.getEndpointBZ());
        if (!player.inventory.addItemStackToInventory(stack))
          player.dropPlayerItemWithRandomChoice(stack, false);
      } else if (m.action == STOP) p.stopAndReturn(player);
      else if (m.action == DELETE) {
        java.util.UUID id = p.getPlatformId();
        if (p.dismantle(player))
          ru.givler.mbo.network.PacketManager.INSTANCE.sendToDimension(
