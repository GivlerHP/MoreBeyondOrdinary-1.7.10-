...
public class PacketSpawnParticle implements IMessage {
  ...
  public PacketSpawnParticle(
      EnumParticleType type, double x, double y, double z, double motX, double motY, double motZ) {
    this.type = type;
    this.x = x;
    this.y = y;
    this.z = z;
  ...
  public static void send(
    ...
    NetworkRegistry.TargetPoint point =
        new NetworkRegistry.TargetPoint(
            player.dimension, player.posX, player.posY, player.posZ, 64.0);
    ...
    for (int i = 0; i < count; i++) {
      ...
      PacketSpawnParticle pkt =
          new PacketSpawnParticle(
              type, player.posX + ox, baseY + oy, player.posZ + oz, motX, motY, motZ);
