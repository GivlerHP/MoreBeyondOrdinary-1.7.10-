...
public class PacketApplyLockTemplate implements IMessage {
  private int x, y, z;
  private NBTTagCompound template;

  public PacketApplyLockTemplate() {}

  public PacketApplyLockTemplate(int x, int y, int z, NBTTagCompound template) {
    this.x = x;
    this.y = y;
    this.z = z;
  ...
  public void fromBytes(ByteBuf b) {
    ...
    y = b.readInt();
    z = b.readInt();
    template = ByteBufUtils.readTag(b);
  }
  ...
  public void toBytes(ByteBuf b) {
    ...
    b.writeInt(y);
    b.writeInt(z);
    ByteBufUtils.writeTag(b, template);
  }
  ...
  public static class Handler implements IMessageHandler<PacketApplyLockTemplate, IMessage> {
    ...
    public IMessage onMessage(PacketApplyLockTemplate m, MessageContext ctx) {
      ...
      chest
          .getLockData()
          .setDifficulty(
              LockDifficulty.byOrdinal(m.template.getInteger("Difficulty")), p.worldObj.rand);
