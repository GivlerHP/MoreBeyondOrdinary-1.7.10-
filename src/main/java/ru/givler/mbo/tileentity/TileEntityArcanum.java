...
public class TileEntityArcanum extends TileEntity implements ISidedInventory {
  ...
  private boolean canAccept(ItemStack result) {
    ...
    if (!current.isItemEqual(result) || !ItemStack.areItemStackTagsEqual(current, result))
      return false;
  ...
  private boolean consumeFuel() {
    ...
    if (fuel.stackSize <= 0) {
      inventory[FUEL_SLOT] =
          consumed.getItem().hasContainerItem(consumed)
              ? consumed.getItem().getContainerItem(consumed)
              : null;
    }
    return true;
  }
  ...
  private void dropRemainder(ItemStack stack) {
    worldObj.spawnEntityInWorld(
        new EntityItem(worldObj, xCoord + .5, yCoord + .75, zCoord + .5, stack));
  }
  ...
  public void setCurrentItemBurnTime(int value) {
  ...
  public ItemStack[] getCraftingMatrix() {
    return Arrays.copyOfRange(inventory, 0, INPUT_END);
  }
  ...
  public ItemStack decrStackSize(int index, int count) {
    ...
    if (stack.stackSize <= count) {
      ...
      if (stack.stackSize <= 0) inventory[index] = null;
    }
    markDirty();
    return result;
  }
  ...
  public ItemStack getStackInSlotOnClosing(int index) {
    ItemStack result = inventory[index];
    inventory[index] = null;
    return result;
  }
  ...
  public void setInventorySlotContents(int index, ItemStack stack) {
    ...
    if (stack != null)
      stack.stackSize =
          Math.min(stack.stackSize, Math.min(getInventoryStackLimit(), stack.getMaxStackSize()));
    markDirty();
  }
