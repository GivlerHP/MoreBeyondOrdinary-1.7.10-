...
public final class LockData {
  ...
  public boolean isLocked() {
    return difficulty != LockDifficulty.NONE && locked;
  }

  public int getRelockDelaySec() {
    return relockDelaySec;
  }
  ...
  public long getRelockAtEpochSec() {
    return relockAtEpochSec;
  }
  ...
  public void setRelockDelaySec(int value) {
    relockDelaySec = Math.max(0, value);
  }

  public void setPlayerRadius(int value) {
    playerRadius = Math.max(0, Math.min(128, value));
  }
  ...
  public boolean hasCompleted(UUID player) {
    return attempts.containsKey(player) && attempts.get(player) >= pins.length;
  }

  public void clearAttempt(UUID player) {
    attempts.remove(player);
    attemptActivity.remove(player);
  }
  ...
  public boolean shouldRelock(TileEntity tile) {
    return difficulty != LockDifficulty.NONE
        && !locked
        && !hasActiveAttempt()
        && relockAtEpochSec > 0L
        && now() >= relockAtEpochSec
        && !hasPlayerNearby(tile);
  }
  ...
  public void readFromNBT(NBTTagCompound tag) {
    ...
    relockDelaySec =
        tag.hasKey("mboRelockDelay") ? Math.max(0, tag.getInteger("mboRelockDelay")) : 30;
    playerRadius =
        tag.hasKey("mboLockPlayerRadius") ? Math.max(0, tag.getInteger("mboLockPlayerRadius")) : 10;
  }
