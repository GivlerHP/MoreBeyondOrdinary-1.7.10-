...
public class EntityMBOBoat extends Entity {
  ...
  public int getTimeSinceHit() {
    return dataWatcher.getWatchableObjectInt(HIT);
  }

  public void setTimeSinceHit(int value) {
    dataWatcher.updateObject(HIT, Integer.valueOf(value));
  }

  public int getForwardDirection() {
    return dataWatcher.getWatchableObjectInt(FORWARD);
  }

  public void setForwardDirection(int value) {
    dataWatcher.updateObject(FORWARD, Integer.valueOf(value));
  }

  public float getDamageTaken() {
    return dataWatcher.getWatchableObjectFloat(DAMAGE);
  }

  public void setDamageTaken(float value) {
    dataWatcher.updateObject(DAMAGE, Float.valueOf(value));
  }
  ...
  public float getRowingTime(int side, float partial) {
    return getPaddleState(side)
        ? (float)
            MathHelper.denormalizeClamp(
                paddlePositions[side] - 0.3926991D, paddlePositions[side], partial)
        : 0;
  }

  protected boolean hasSecondSeat() {
    return true;
  }

  public EntityMBOBoatSeat getSecondSeat() {
    return secondSeat;
  }

  public void setSecondSeat(EntityMBOBoatSeat seat) {
    secondSeat = seat;
  }
  ...
  public List<EntityLivingBase> getPassengers() {
    ...
    if (riddenByEntity instanceof EntityLivingBase)
      passengers.add((EntityLivingBase) riddenByEntity);
  ...
  public void onUpdate() {
    ...
    if (!worldObj.isRemote && outOfControlTicks >= 60 && riddenByEntity != null)
  ...
  private Status getBoatStatus() {
    ...
    Block below =
        worldObj.getBlock(
            ...
            MathHelper.floor_double(boundingBox.minY - 0.01D),
  ...
  private Status getUnderwaterStatus() {
    ...
    for (int x = MathHelper.floor_double(boundingBox.minX);
        x < MathHelper.ceiling_double_int(boundingBox.maxX);
        x++)
      for (int z = MathHelper.floor_double(boundingBox.minZ);
          z < MathHelper.ceiling_double_int(boundingBox.maxZ);
          z++)
        if (worldObj.getBlock(x, y, z).getMaterial() == Material.water
            && top < liquidHeight(worldObj, x, y, z))
  ...
  private float getWaterLevelAbove() {
    ...
    for (int test = y; test < y + 5; test++)
      if (worldObj
              .getBlock(MathHelper.floor_double(posX), test, MathHelper.floor_double(posZ))
              .getMaterial()
          != Material.water) return test;
    return y + 5;
  }
  ...
  private static float liquidHeight(World world, int x, int y, int z) {
    ...
    return y
        + ((meta % 8) == 0 && world.getBlock(x, y + 1, z).getMaterial() == Material.water
            ? 1F
            : 1F - BlockLiquid.getLiquidHeightPercent(meta));
  }
  ...
  public void updateSecondPassenger(Entity passenger) {
    updatePassenger(passenger, -0.6F);
  }
  ...
  protected ItemStack getBoatDrop() {
    ...
    return new ItemStack(
        bop ? BoatRegistry.bopBoats : BoatRegistry.vanillaBoats,
        1,
        bop ? getBoatType() - 6 : getBoatType());
  }
  ...
  protected void writeEntityToNBT(NBTTagCompound tag) {
    tag.setByte("MBOBoatType", (byte) getBoatType());
  }

  protected void readEntityFromNBT(NBTTagCompound tag) {
    setBoatType(tag.getByte("MBOBoatType") & 255);
  }
