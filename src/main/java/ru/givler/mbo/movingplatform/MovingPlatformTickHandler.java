...
public final class MovingPlatformTickHandler {
  private static final Map<World, Set<EntityMovingPlatform>> ACTIVE =
      new WeakHashMap<World, Set<EntityMovingPlatform>>();
  private static final Map<World, Map<UUID, Integer>> CARRIERS =
      new WeakHashMap<World, Map<UUID, Integer>>();
  private static final Map<World, Map<Entity, Integer>> ENTITY_CARRIERS =
      new WeakHashMap<World, Map<Entity, Integer>>();
  private static final Map<World, Map<Entity, RideOffset>> CLIENT_OFFSETS =
      new WeakHashMap<World, Map<Entity, RideOffset>>();

  public static void track(EntityMovingPlatform platform) {
    if (platform.worldObj == null) return;
    Set<EntityMovingPlatform> set = ACTIVE.get(platform.worldObj);
    if (set == null) {
      set = Collections.newSetFromMap(new IdentityHashMap<EntityMovingPlatform, Boolean>());
      ACTIVE.put(platform.worldObj, set);
    }
    set.add(platform);
  }
  ...
  public static void untrack(EntityMovingPlatform platform) {
    ...
    Set<EntityMovingPlatform> set = ACTIVE.get(world);
    if (set != null) set.remove(platform);
    int entityId = platform.getEntityId();
    Map<UUID, Integer> players = CARRIERS.get(world);
    if (players != null) removeCarrierId(players, entityId);
    Map<Entity, Integer> entities = ENTITY_CARRIERS.get(world);
    if (entities != null) removeCarrierId(entities, entityId);
    Map<Entity, RideOffset> offsets = CLIENT_OFFSETS.get(world);
    if (offsets != null) {
      for (Iterator<Map.Entry<Entity, RideOffset>> it = offsets.entrySet().iterator();
          it.hasNext(); ) if (it.next().getValue().platformId == entityId) it.remove();
    }
  }

  private static <K> void removeCarrierId(Map<K, Integer> map, int entityId) {
    for (Iterator<Map.Entry<K, Integer>> it = map.entrySet().iterator(); it.hasNext(); )
      if (it.next().getValue().intValue() == entityId) it.remove();
  }
  ...
  public void onServerTick(TickEvent.ServerTickEvent event) {
    ...
    for (WorldServer world : server.worldServers) if (world != null) processWorld(world);
  }

  public static void processWorld(World world) {
    if (world.isRemote) return;
    Set<EntityMovingPlatform> platforms = ACTIVE.get(world);
    ...
    for (Iterator<EntityMovingPlatform> it = platforms.iterator(); it.hasNext(); ) {
      EntityMovingPlatform platform = it.next();
      if (platform == null || platform.isDead || platform.worldObj != world) {
        it.remove();
        continue;
      }
  ...
  public static void processClientPlayer(World world, EntityPlayer player) {
    ...
    for (EntityMovingPlatform platform : platforms)
      if (platform != null
          && !platform.isDead
          && platform.isCollisionActive()
          && player.boundingBox.intersectsWith(bounds(platform).expand(1.5D, 2.5D, 1.5D)))
        processPlayer(player, platform, carriers);
    Map<Entity, Integer> entityCarriers = entityCarriers(world);
    Map<Entity, RideOffset> offsets = clientOffsets(world);
    for (EntityMovingPlatform platform : platforms)
      if (platform != null && !platform.isDead && platform.isCollisionActive()) {
        AxisAlignedBB broad = bounds(platform).expand(1.5D, 2.5D, 1.5D);
        for (Object object : world.getEntitiesWithinAABBExcludingEntity(platform, broad))
          if (object instanceof Entity
            ...
            if (!entity.isDead
                && entity.boundingBox != null
                && entity.boundingBox.intersectsWith(broad))
              processEntity(entity, platform, entityCarriers, offsets, true);
          }
      }
  }

  private static Map<Entity, Integer> entityCarriers(World world) {
    Map<Entity, Integer> map = ENTITY_CARRIERS.get(world);
    if (map == null) {
      map = new WeakHashMap<Entity, Integer>();
      ENTITY_CARRIERS.put(world, map);
    }
    return map;
  }

  private static Map<Entity, RideOffset> clientOffsets(World world) {
    Map<Entity, RideOffset> map = CLIENT_OFFSETS.get(world);
    if (map == null) {
      map = new WeakHashMap<Entity, RideOffset>();
      CLIENT_OFFSETS.put(world, map);
    }
    return map;
  }

  private static AxisAlignedBB bounds(EntityMovingPlatform p) {
    double minX = Math.min(p.prevPosX, p.posX),
        minY = Math.min(p.prevPosY, p.posY),
        minZ = Math.min(p.prevPosZ, p.posZ);
    double maxX = Math.max(p.prevPosX, p.posX) + p.getSizeX(),
        maxY = Math.max(p.prevPosY, p.posY) + p.getSizeY(),
        maxZ = Math.max(p.prevPosZ, p.posZ) + p.getSizeZ();
  ...
  private static void processEntity(
      ...
      Map<Entity, Integer> carriers,
      Map<Entity, RideOffset> offsets,
      boolean clientOnly) {
    double dx = platform.posX - platform.prevPosX,
        dy = platform.posY - platform.prevPosY,
        dz = platform.posZ - platform.prevPosZ;
    Integer previous = carriers.get(entity);
    if (previous != null && previous.intValue() == platform.getEntityId() && entity.motionY > .1D) {
    ...
    if (clientOnly
        && previous != null
        && previous.intValue() == platform.getEntityId()
        && ride != null
        && ride.platformId == platform.getEntityId()) {
      ride.x += entity.motionX;
      ride.z += entity.motionZ;
      entity.setPosition(platform.posX + ride.x, entity.posY, platform.posZ + ride.z);
    }
    ...
    for (PlatformBlock block : platform.getTopBlocks()) {
      ...
      boolean remembered =
          previous != null
              && previous.intValue() == platform.getEntityId()
              && newGap >= -.65D - Math.abs(dy)
              && newGap <= .3D + Math.abs(dy);
      boolean landing =
          oldGap >= -.12D && newGap <= .18D && newGap >= -.55D && entity.motionY <= .2D;
    ...
    if (previous != null && previous.intValue() == platform.getEntityId()) {
      carriers.remove(entity);
      if (offsets != null) offsets.remove(entity);
    }
    ...
    for (PlatformBlock block : platform.getOuterBlocks()) {
      ...
      double pushX =
          dx > 0
              ? box.maxX - entity.boundingBox.minX
              : dx < 0 ? box.minX - entity.boundingBox.maxX : 0D;
      double pushY =
          dy > 0
              ? box.maxY - entity.boundingBox.minY
              : dy < 0 ? box.minY - entity.boundingBox.maxY : 0D;
      double pushZ =
          dz > 0
              ? box.maxZ - entity.boundingBox.minZ
              : dz < 0 ? box.minZ - entity.boundingBox.maxZ : 0D;
      entity.moveEntity(pushX, pushY, pushZ);
      boolean blocked =
          Math.abs(entity.posX - beforeX - pushX) > .02D
              || Math.abs(entity.posY - beforeY - pushY) > .02D
              || Math.abs(entity.posZ - beforeZ - pushZ) > .02D;
      if (blocked && entity instanceof EntityLivingBase && entity.ticksExisted % 10 == 0)
        entity.attackEntityFrom(DamageSource.inWall, 4F);
      if (dy > 0) {
        entity.onGround = true;
        entity.fallDistance = 0;
      }
  ...
  private static void processPlayer(
      EntityPlayer player, EntityMovingPlatform platform, Map<UUID, Integer> carriers) {
    UUID id = player.getUniqueID();
    Integer previous = carriers.get(id);
    double feet = feet(player), head = player.boundingBox.maxY;
    double dx = platform.posX - platform.prevPosX,
        dy = platform.posY - platform.prevPosY,
        dz = platform.posZ - platform.prevPosZ;
    if (previous != null
    ...
    for (PlatformBlock block : platform.getTopBlocks()) {
      ...
      boolean remembered =
          previous != null
              && previous.intValue() == platform.getEntityId()
              && player.motionY <= .05D
              && newGap >= -.55D - Math.abs(dy)
              && newGap <= .12D + Math.abs(dy);
      boolean contact = newGap >= -.38D && newGap <= .12D && player.motionY <= 0D;
      if ((descendingCrossing || remembered || contact) && Math.abs(newGap) < best) {
        chosen = block;
        best = Math.abs(newGap);
      }
    }
    if (chosen != null) {
  ...
  private static double feet(Entity p) {
    return p.boundingBox.minY;
  }

  private static double previousFeet(Entity p) {
    return feet(p) + (p.prevPosY - p.posY);
  }

  private static void snapFeet(Entity p, double target) {
    double d = target - feet(p);
    if (Math.abs(d) > 1E-9) p.setPosition(p.posX, p.posY + d, p.posZ);
  }

  private static void snapHead(Entity p, double target) {
    double d = target - p.boundingBox.maxY;
    if (Math.abs(d) > 1E-9) p.setPosition(p.posX, p.posY + d, p.posZ);
  }
