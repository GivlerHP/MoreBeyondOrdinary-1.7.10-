...
public class TileEntityPlatformStation extends TileEntity {
    ...
    public void updatePower(){
        ...
        if(now!=powered&&platformId!=null){EntityMovingPlatform platform=findPlatform();
            if(platform==null&&hasEndpoints&&worldObj.provider.dimensionId==dimension){worldObj.getChunkFromBlockCoords(aX,aZ);worldObj.getChunkFromBlockCoords(bX,bZ);platform=findPlatform();}
            if(platform!=null)platform.start(targetB,null);
        }
        powered=now;markDirty();
    }
    private EntityMovingPlatform findPlatform(){for(Object o:worldObj.loadedEntityList)if(o instanceof EntityMovingPlatform&&platformId.equals(((EntityMovingPlatform)o).getPlatformId()))return (EntityMovingPlatform)o;return null;}
    @Override public void readFromNBT(NBTTagCompound tag){super.readFromNBT(tag);readLink(tag);powered=tag.getBoolean("Powered");}
    @Override public void writeToNBT(NBTTagCompound tag){super.writeToNBT(tag);if(platformId!=null)tag.setString("PlatformId",platformId.toString());tag.setBoolean("TargetB",targetB);tag.setBoolean("Powered",powered);tag.setInteger("Dimension",dimension);if(hasEndpoints){tag.setInteger("AX",aX);tag.setInteger("AY",aY);tag.setInteger("AZ",aZ);tag.setInteger("BX",bX);tag.setInteger("BY",bY);tag.setInteger("BZ",bZ);}}
}
