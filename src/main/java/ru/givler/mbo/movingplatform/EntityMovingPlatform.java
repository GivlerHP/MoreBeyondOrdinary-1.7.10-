...
public class EntityMovingPlatform extends Entity implements IEntityAdditionalSpawnData {
  ...
  public static EntityMovingPlatform create(
      World world, EntityPlayer owner, int minX, int minY, int minZ, int maxX, int maxY, int maxZ) {
    EntityMovingPlatform entity = new EntityMovingPlatform(world);
    entity.homeX = minX;
    entity.homeY = minY;
    entity.homeZ = minZ;
    entity.sizeX = maxX - minX + 1;
    entity.sizeY = maxY - minY + 1;
    entity.sizeZ = maxZ - minZ + 1;
  ...
  public boolean rebuild(EntityPlayer feedback) {
    ...
    if (found.isEmpty()) {
      if (feedback != null)
        feedback.addChatMessage(new ChatComponentTranslation("mbo.platform.error.empty"));
      return false;
    }
    blocks.clear();
    blocks.addAll(found);
  ...
  private PlatformBlock findSavedBlock(int x, int y, int z) {
    for (PlatformBlock saved : blocks)
      if (saved.x == x && saved.y == y && saved.z == z) return saved;
    return null;
  }
  ...
  public boolean dismantle(EntityPlayer feedback) {
    ...
    for (Object object : new java.util.ArrayList(worldObj.loadedEntityList))
      if (object instanceof EntityMovingPlatform) {
        EntityMovingPlatform other = (EntityMovingPlatform) object;
        if (platformId.equals(other.platformId)) other.setDead();
      }
    return true;
  }
  ...
  public boolean isMoving() {
    return state == MOVING_TO_A || state == MOVING_TO_B;
  }
  ...
  public PlatformDirection getDirection() {
    return PlatformDirection.byOrdinal(direction);
  }
  ...
  public int getDirectionIndex() {
    return pendingConfiguration ? pendingDirection : direction;
  }

  public int getDistance() {
    return pendingConfiguration ? pendingDistance : distance;
  }

  public int getDurationTicks() {
    return pendingConfiguration ? pendingDurationTicks : durationTicks;
  }
  ...
  public int getReturnMode() {
    return pendingConfiguration ? pendingReturnMode : returnMode;
  }

  public int getDelayTicks() {
    return pendingConfiguration ? pendingDelayTicks : delayTicks;
  }
  ...
  public int getEndpointAZ() {
    return floor(homeZ);
  }

  public int getEndpointBX() {
    return floor(homeX) + getDirection().x * distance;
  }

  public int getEndpointBY() {
    return floor(homeY) + getDirection().y * distance;
  }

  public int getEndpointBZ() {
    return floor(homeZ) + getDirection().z * distance;
  }
  ...
  public void configure(
    ...
    if (isMoving()) {
      ...
      pendingDurationTicks = newDuration;
      pendingReturnMode = newReturnMode;
      pendingDelayTicks = newDelay;
      return;
    }
  ...
  private void applyPendingConfiguration() {
    ...
    durationTicks = pendingDurationTicks;
    returnMode = pendingReturnMode;
    delayTicks = pendingDelayTicks;
    pendingConfiguration = false;
    dataWatcher.updateObject(20, Integer.valueOf(state));
    dataWatcher.updateObject(21, Integer.valueOf(direction));
    dataWatcher.updateObject(22, Integer.valueOf(distance));
    dataWatcher.updateObject(23, Integer.valueOf(durationTicks));
  }
  ...
  public void readPlatformTag(NBTTagCompound tag) {
  ...
  protected void readEntityFromNBT(NBTTagCompound tag) {
    ...
    sizeX = tag.getInteger("SizeX");
    sizeY = tag.getInteger("SizeY");
    sizeZ = tag.getInteger("SizeZ");
    direction = tag.getInteger("Direction");
    distance = tag.getInteger("Distance");
    durationTicks = tag.getInteger("Duration");
    state = tag.getInteger("State");
    motionTick = tag.getInteger("MotionTick");
    ...
    pendingReturnMode = tag.getInteger("PendingReturnMode");
  ...
  protected void writeEntityToNBT(NBTTagCompound tag) {
    ...
    tag.setInteger("PendingReturnMode", pendingReturnMode);
    tag.setInteger("PendingDelay", pendingDelayTicks);
    NBTTagList list = new NBTTagList();
    for (PlatformBlock b : blocks) list.appendTag(b.write());
    tag.setTag("Blocks", list);
  }

  private void setEndpointPosition() {
    PlatformDirection d = getDirection();
    int o = state == STOPPED_B ? distance : 0;
    setPosition(homeX + d.x * o, homeY + d.y * o, homeZ + d.z * o);
  }

  private static int floor(double v) {
    return (int) Math.floor(v + 1.0E-5D);
  }

  private int worldTick() {
    return (int) (worldObj.getTotalWorldTime() & 0x7fffffffL);
  }
  ...
  private void rebuildCollisionCache() {
    ...
    java.util.HashMap<Long, PlatformBlock> indexed = new java.util.HashMap<Long, PlatformBlock>();
    for (PlatformBlock b : blocks) {
      occupied.add(blockKey(b.x, b.y, b.z));
      indexed.put(blockKey(b.x, b.y, b.z), b);
    }
    for (PlatformBlock b : blocks) {
      PlatformBlock above = indexed.get(blockKey(b.x, b.y + 1, b.z)),
          below = indexed.get(blockKey(b.x, b.y - 1, b.z));
      boolean top = above == null || !above.block.isNormalCube(),
          bottom = below == null || !below.block.isNormalCube();
      if (top) topBlocks.add(b);
      if (bottom) bottomBlocks.add(b);
      if (top
          || bottom
          || !occupied.contains(blockKey(b.x + 1, b.y, b.z))
          || !occupied.contains(blockKey(b.x - 1, b.y, b.z))
          || !occupied.contains(blockKey(b.x, b.y, b.z + 1))
          || !occupied.contains(blockKey(b.x, b.y, b.z - 1))) outerBlocks.add(b);
    }
  }
  ...
  public void setPositionAndRotation2(
      double x, double y, double z, float yaw, float pitch, int steps) {
    lerpX = x;
    lerpY = y;
    lerpZ = z;
    lerpSteps = 5;
  ...
  public void writeSpawnData(ByteBuf buf) {
    try {
      byte[] bytes = net.minecraft.nbt.CompressedStreamTools.compress(writePlatformTag());
  ...
  public void readSpawnData(ByteBuf buf) {
    try {
      ...
      buf.readBytes(bytes);
      readPlatformTag(
          net.minecraft.nbt.CompressedStreamTools.func_152457_a(
              bytes, new net.minecraft.nbt.NBTSizeTracker(MAX_SPAWN_NBT_BYTES)));
  ...
  @Override
  public void setDead() {
    MovingPlatformTickHandler.untrack(this);
    super.setDead();
  }
}
