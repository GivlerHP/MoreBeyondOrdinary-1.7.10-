...
public class BlockButtonTransformer implements IClassTransformer, Opcodes {
  ...
  public byte[] transform(String name, String transformedName, byte[] bytes) {
    ...
    for (MethodNode method : node.methods) {
      if (matches(
          node.name,
          method,
          "(Lnet/minecraft/world/World;IIII)Z",
          "canPlaceBlockOnSide",
          "func_149707_d")) {
        ...
        changed = true;
          ...
          node.name, method, "(Lnet/minecraft/world/World;IIII)V", "func_150042_a")) {
        injectNotifyOverride(method);
        changed = true;
      } else if (matches(
          node.name,
          method,
          "(Lnet/minecraft/world/IBlockAccess;IIII)I",
  ...
  private static void injectIntOverride(
      MethodNode method, String hook, String hookDesc, int returnOpcode) {
  ...
  private static void injectPlacedOverride(MethodNode method) {
    ...
    code.add(
        new MethodInsnNode(
            INVOKESTATIC, HOOKS, "placedMeta", "(Lnet/minecraft/world/World;IIII)I", false));
    int local = method.maxLocals++;
    code.add(new VarInsnNode(ISTORE, local));
    LabelNode vanilla = new LabelNode();
    code.add(new VarInsnNode(ILOAD, local));
  ...
  private static void injectBoundsOverride(MethodNode method) {
    ...
    code.add(
        new MethodInsnNode(
            INVOKESTATIC, HOOKS, "setBounds", "(Lnet/minecraft/block/BlockButton;I)Z", false));
    LabelNode vanilla = new LabelNode();
    ...
    for (AbstractInsnNode instruction = method.instructions.getFirst();
      ...
      code.add(
          new MethodInsnNode(
              ...
              HOOKS,
              "placedBy",
              "(Lnet/minecraft/world/World;IIILnet/minecraft/entity/EntityLivingBase;)V",
              false));
  ...
  private static void addPlacedByOverride(ClassNode node, String methodName) {
    String desc =
        "(Lnet/minecraft/world/World;IIILnet/minecraft/entity/EntityLivingBase;"
  ...
  private static void injectStrongPower(MethodNode method) {
    ...
    code.add(
        new MethodInsnNode(
            ...
            HOOKS,
            "strongPower",
            "(Lnet/minecraft/block/BlockButton;Lnet/minecraft/world/IBlockAccess;IIII)I",
            false));
    int local = method.maxLocals++;
    code.add(new VarInsnNode(ISTORE, local));
    LabelNode vanilla = new LabelNode();
