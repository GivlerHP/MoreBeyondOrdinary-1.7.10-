...
public final class SmoothOpeningTransformer implements IClassTransformer, Opcodes {
  ...
  public byte[] transform(String name, String transformedName, byte[] bytes) {
    ...
    try {
      ...
      System.err.println(
          "[MBO ASM] Smooth-opening patch failed for "
              + transformedName
              + "; keeping the original class: "
              + error);
  ...
  private byte[] patchCarpenterRenderer(byte[] bytes) {
    ClassNode node = read(bytes);
    String desc =
        "(Lnet/minecraft/item/ItemStack;IIIDDDDDD[Lnet/minecraftforge/common/util/ForgeDirection;)V";
    for (MethodNode m : node.methods) {
      ...
      code.add(new VarInsnNode(DLOAD, 5));
      code.add(new VarInsnNode(DLOAD, 7));
      code.add(new VarInsnNode(DLOAD, 9));
      code.add(new VarInsnNode(DLOAD, 11));
      code.add(new VarInsnNode(DLOAD, 13));
      code.add(new VarInsnNode(DLOAD, 15));
      code.add(new VarInsnNode(ALOAD, 17));
      code.add(
          new MethodInsnNode(
              INVOKESTATIC,
              HOOK,
              "splitCarpenterGateCuboid",
              "(Ljava/lang/Object;Lnet/minecraft/item/ItemStack;IIIDDDDDD[Lnet/minecraftforge/common/util/ForgeDirection;)Z",
              false));
      LabelNode normal = new LabelNode();
      code.add(new JumpInsnNode(IFEQ, normal));
    ...
    System.err.println("[MBO ASM] Carpenter cuboid renderer not found");
    return bytes;
  }
    ...
    for (MethodNode m : node.methods) {
      ...
      head.add(
          new MethodInsnNode(
              ...
              HOOK,
              "begin",
              "(Lnet/minecraft/client/renderer/RenderBlocks;Lnet/minecraft/block/Block;III)Z",
              false));
      ...
      for (AbstractInsnNode ins = m.instructions.getFirst(); ins != null; ins = ins.getNext()) {
        if (ins.getOpcode() == IRETURN) {
          InsnList end = new InsnList();
          end.add(new MethodInsnNode(INVOKESTATIC, HOOK, "end", "()V", false));
          m.instructions.insertBefore(ins, end);
        }
      }
      System.out.println("[MBO ASM] Patched RenderBlocks vertex animation scope");
      return write(node);
    }
    System.err.println("[MBO ASM] RenderBlocks animation method not found");
    return bytes;
  }
  ...
  private byte[] patchTessellator(byte[] bytes) {
    ClassNode node = read(bytes);
    String desc = "(DDDDD)V";
    boolean vertexPatched = false;
    for (MethodNode m : node.methods) {
      if (matches(node.name, m, desc, "addVertexWithUV", "func_78374_a")) {
        int local = m.maxLocals;
        m.maxLocals += 1;
        InsnList code = new InsnList();
        code.add(new VarInsnNode(DLOAD, 1));
        code.add(new VarInsnNode(DLOAD, 3));
        code.add(new VarInsnNode(DLOAD, 5));
        ...
        code.add(new InsnNode(DALOAD));
        code.add(new VarInsnNode(DSTORE, 1));
        code.add(new VarInsnNode(ALOAD, local));
        code.add(new InsnNode(ICONST_1));
        code.add(new InsnNode(DALOAD));
        code.add(new VarInsnNode(DSTORE, 3));
        code.add(new VarInsnNode(ALOAD, local));
        code.add(new InsnNode(ICONST_2));
        code.add(new InsnNode(DALOAD));
        code.add(new VarInsnNode(DSTORE, 5));
        m.instructions.insert(code);
        vertexPatched = true;
        ...
        code.add(new VarInsnNode(FLOAD, 3));
        code.add(new MethodInsnNode(INVOKESTATIC, HOOK, "correctColor", "(FFF)[F", false));
        code.add(new VarInsnNode(ASTORE, local));
        code.add(new VarInsnNode(ALOAD, local));
        code.add(new InsnNode(ICONST_0));
        code.add(new InsnNode(FALOAD));
        code.add(new VarInsnNode(FSTORE, 1));
        code.add(new VarInsnNode(ALOAD, local));
        code.add(new InsnNode(ICONST_1));
        code.add(new InsnNode(FALOAD));
        code.add(new VarInsnNode(FSTORE, 2));
        code.add(new VarInsnNode(ALOAD, local));
        code.add(new InsnNode(ICONST_2));
        code.add(new InsnNode(FALOAD));
        code.add(new VarInsnNode(FSTORE, 3));
        m.instructions.insert(code);
    ...
    if (vertexPatched) {
      System.out.println("[MBO ASM] Patched Tessellator vertex animation and color");
      return write(node);
    }
    System.err.println("[MBO ASM] Tessellator UV vertex method not found");
    return bytes;
  }
  ...
  private static ClassNode read(byte[] b) {
