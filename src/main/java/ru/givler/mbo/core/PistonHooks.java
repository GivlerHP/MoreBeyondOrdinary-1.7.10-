...
/**
 * Back-in-Slime style piston planner. Returning -1 lets the unmodified 1.7.10 piston handle
 * assemblies that do not contain an MBO slime block.
 */
public final class PistonHooks {
  private PistonHooks() {}

  public static int handleEvent(
      BlockPistonBase piston, World world, int x, int y, int z, int event, int side) {
    ...
    if (event == 0) {
      ...
      world.setBlock(x + dx, y + dy, z + dz, Blocks.piston_extension, side | (sticky ? 8 : 0), 4);
      world.setTileEntity(
          x + dx,
          y + dy,
          z + dz,
          BlockPistonMoving.getTileEntity(
              Blocks.piston_head, side | (sticky ? 8 : 0), side, true, false));
      world.setBlockMetadataWithNotify(x, y, z, side | 8, 3);
      world.playSoundEffect(
          x + .5D, y + .5D, z + .5D, "tile.piston.out", .5F, world.rand.nextFloat() * .25F + .6F);
      return 1;
    }
    ...
    if ((event == 1 || event == -1) && sticky) {
      ...
      world.setTileEntity(
          x, y, z, BlockPistonMoving.getTileEntity(piston, side, side, false, true));
      world.setBlockToAir(x + dx, y + dy, z + dz);
      move(world, moves, Facing.oppositeSide[side]);
      world.playSoundEffect(
          x + .5D, y + .5D, z + .5D, "tile.piston.in", .5F, world.rand.nextFloat() * .15F + .6F);
      return 1;
    }
    return -1;
  }
  ...
  private static boolean move(World world, List<Move> moves, final int side) {
    ...
    for (Move move : moves) {
      ...
      if (move.block.getMobilityFlag() == 1) {
        ...
        move.block.dropBlockAsItemWithChance(world, move.x, move.y, move.z, move.meta, chance, 0);
        world.setBlockToAir(nx, ny, nz);
      } else {
        world.setBlock(nx, ny, nz, Blocks.piston_extension, move.meta, 4);
        world.setTileEntity(
            nx, ny, nz, BlockPistonMoving.getTileEntity(move.block, move.meta, side, true, false));
      }
    }
    return true;
  }
