...
public final class PaneTransformer implements IClassTransformer, Opcodes {
    ...
    private byte[] patchPane(byte[] bytes) {
        ...
        for (MethodNode method : node.methods) {
            ...
            if ("(Lnet/minecraft/world/IBlockAccess;IIILnet/minecraftforge/common/util/ForgeDirection;)Z".equals(desc)
                ...
                code.add(new MethodInsnNode(INVOKESTATIC, PANE_HOOK, "canConnect",
                        "(Lnet/minecraft/block/BlockPane;Lnet/minecraft/world/IBlockAccess;IIILnet/minecraftforge/common/util/ForgeDirection;)Z", false));
                ...
                code.add(new MethodInsnNode(INVOKESTATIC, PANE_HOOK, "setIsolatedBounds",
                        "(Lnet/minecraft/block/BlockPane;Lnet/minecraft/world/IBlockAccess;III)Z", false));
                LabelNode vanilla = new LabelNode();
                code.add(new JumpInsnNode(IFEQ, vanilla)); code.add(new InsnNode(RETURN)); code.add(vanilla);
                method.instructions.insert(code);
                patched++;
            } else if ("(Lnet/minecraft/world/World;IIILnet/minecraft/util/AxisAlignedBB;Ljava/util/List;Lnet/minecraft/entity/Entity;)V".equals(desc)
                    && mappedName(node, method, "addCollisionBoxesToList", "func_149743_a")) {
                InsnList code = new InsnList();
                code.add(new VarInsnNode(ALOAD, 0)); code.add(new VarInsnNode(ALOAD, 1));
                code.add(new VarInsnNode(ILOAD, 2)); code.add(new VarInsnNode(ILOAD, 3)); code.add(new VarInsnNode(ILOAD, 4));
                code.add(new VarInsnNode(ALOAD, 5)); code.add(new VarInsnNode(ALOAD, 6));
                code.add(new MethodInsnNode(INVOKESTATIC, PANE_HOOK, "addIsolatedCollision",
                        "(Lnet/minecraft/block/BlockPane;Lnet/minecraft/world/World;IIILnet/minecraft/util/AxisAlignedBB;Ljava/util/List;)Z", false));
                LabelNode vanilla = new LabelNode();
                code.add(new JumpInsnNode(IFEQ, vanilla)); code.add(new InsnNode(RETURN)); code.add(vanilla);
                method.instructions.insert(code);
                patched++;
    ...
    private byte[] patchRenderer(byte[] bytes) {
        ...
        for (MethodNode method : node.methods) {
            String desc = FMLDeobfuscatingRemapper.INSTANCE.mapMethodDesc(method.desc);
            boolean regular = "(Lnet/minecraft/block/BlockPane;III)Z".equals(desc)
                    && mappedName(node, method, "renderBlockPane", "func_147767_a");
            boolean stained = "(Lnet/minecraft/block/Block;III)Z".equals(desc)
            ...
            code.add(new VarInsnNode(ILOAD, 2)); code.add(new VarInsnNode(ILOAD, 3)); code.add(new VarInsnNode(ILOAD, 4));
            code.add(new MethodInsnNode(INVOKESTATIC, RENDER_HOOK, "renderIsolated",
                    "(Lnet/minecraft/client/renderer/RenderBlocks;Lnet/minecraft/block/BlockPane;III)Z", false));
            LabelNode vanilla = new LabelNode();
            code.add(new JumpInsnNode(IFEQ, vanilla)); code.add(new InsnNode(ICONST_1)); code.add(new InsnNode(IRETURN)); code.add(vanilla);
            method.instructions.insert(code);
            patched++;
        }
        if (patched != 2) {
    ...
    private static boolean mappedName(ClassNode owner, MethodNode method, String mcp, String srg) {
        String mapped = FMLDeobfuscatingRemapper.INSTANCE.mapMethodName(owner.name, method.name, method.desc);
        return mcp.equals(method.name) || srg.equals(method.name) || mcp.equals(mapped) || srg.equals(mapped);
    }
