...
public class ContainerArcanum extends Container {
  ...
  public ContainerArcanum(InventoryPlayer playerInventory, TileEntityArcanum tile) {
    ...
    for (int row = 0; row < 3; row++)
      for (int col = 0; col < 9; col++)
        addSlotToContainer(
            new Slot(playerInventory, col + row * 9 + 9, 8 + col * 18, 106 + row * 18));
    for (int col = 0; col < 9; col++)
      addSlotToContainer(new Slot(playerInventory, col, 8 + col * 18, 164));
  }
  ...
  public void detectAndSendChanges() {
    ...
    for (Object object : crafters) {
      ...
      if (lastItemBurnTime != tile.getCurrentItemBurnTime())
        c.sendProgressBarUpdate(this, 3, tile.getCurrentItemBurnTime());
    ...
    lastBurnTime = tile.getBurnTime();
    lastItemBurnTime = tile.getCurrentItemBurnTime();
