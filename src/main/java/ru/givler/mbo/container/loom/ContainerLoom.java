package ru.givler.mbo.container.loom;
...
import ru.givler.mbo.banner.BannerData;
...
public class ContainerLoom extends Container {
  ...
  public ContainerLoom(InventoryPlayer playerInventory, World world, int x, int y, int z) {
    ...
    this.z = z;
    inputs =
        new InventoryBasic("loom.input", false, 3) {
          @Override
    ...
    addSlotToContainer(
        new Slot(inputs, 0, 13, 26) {
          @Override
          public boolean isItemValid(ItemStack stack) {
            return stack.getItem() == Item.getItemFromBlock(BannerRegistry.banner);
          }
        });
    addSlotToContainer(
        new Slot(inputs, 1, 33, 26) {
          @Override
          public boolean isItemValid(ItemStack stack) {
  ...
  private void updateResult() {
    ...
    output.setInventorySlotContents(
        0,
        BannerData.addPattern(
            banner, BannerPattern.values()[selectedPattern], dye.getItemDamage()));
  }
  ...
  private boolean validTemplate(BannerPattern pattern, ItemStack stack) {
    switch (pattern) {
      case CURLY_BORDER:
        return stack != null
            && stack.getItem() == Item.getItemFromBlock(net.minecraft.init.Blocks.vine);
      case CREEPER:
        return stack != null && stack.getItem() == Items.skull && stack.getItemDamage() == 4;
      case BRICKS:
        return stack != null
            && stack.getItem() == Item.getItemFromBlock(net.minecraft.init.Blocks.brick_block);
      case SKULL:
        return stack != null && stack.getItem() == Items.skull && stack.getItemDamage() == 1;
      case FLOWER:
        return stack != null
            && stack.getItem() == Item.getItemFromBlock(net.minecraft.init.Blocks.red_flower)
            && stack.getItemDamage() == 8;
      case MOJANG:
        return stack != null && stack.getItem() == Items.golden_apple && stack.getItemDamage() == 1;
      default:
        return true;
    }
  }
