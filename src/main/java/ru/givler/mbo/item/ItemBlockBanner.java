...
public class ItemBlockBanner extends ItemBlock {
  ...
  public String getItemStackDisplayName(ItemStack stack) {
    return StatCollector.translateToLocal(
        "item.banner." + COLORS[BannerData.getBaseColor(stack)] + ".name");
  }
  ...
  public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean advanced) {
    ...
    for (int i = 0; i < patterns.tagCount() && i < 6; i++) {
      ...
      if (type != null)
        list.add(StatCollector.translateToLocal("item.banner.pattern." + type.texture));
    }
  }
  ...
  public boolean onItemUse(
    ...
    int meta =
        side == 1 ? MathHelper.floor_double((player.rotationYaw + 180) * 16 / 360 + .5) & 15 : side;
