...
public class BlockLockableChest extends BlockChest {
  ...
  public boolean canPlaceBlockAt(World world, int x, int y, int z) {
    for (int dx = -1; dx <= 1; dx++)
      for (int dz = -1; dz <= 1; dz++) {
        if (Math.abs(dx) + Math.abs(dz) == 1 && world.getBlock(x + dx, y, z + dz) == this)
          return false;
      }
  ...
  public boolean onBlockActivated(
    ...
    if (LockableAccess.isAdminKey(player)) {
      tile.onUnlocked(player);
      return true;
    }
    if (tile.getLockData().isLocked()) {
      if (!LockableAccess.openLockpicking(player, tile))
        LockableAccess.playLockedSound(world, x, y, z);
      return true;
    }
