...
/**
 * Compatibility renderer for modern resource-pack fonts. Supported providers: bitmap, space, unihex
 * and reference. Missing glyphs fall back to 1.7.10.
 */
...
public final class ModernFontRenderer extends FontRenderer
  ...
  private static final ResourceLocation LEGACY_ASCII =
  ...
  public void onResourceManagerReload(IResourceManager manager) {
    ...
    try {
      ...
      spaces.clear();
      LOGGER.warn(
          "Could not load the optional modern font; using the vanilla font fallback", error);
    }
  }

  private void loadFont(IResourceManager manager, ResourceLocation jsonLocation)
      throws IOException {
  ...
  private void loadUnihex(IResourceManager manager, JsonObject provider) throws IOException {
    String value =
        provider.has("hex_file")
            ? provider.get("hex_file").getAsString()
    ...
    while ((entry = zip.getNextEntry()) != null) {
      if (!entry.isDirectory() && entry.getName().endsWith(".hex"))
        parseHex(new BufferedReader(new InputStreamReader(zip, UTF8)));
      zip.closeEntry();
    }
    zip.close();
  }
  ...
  private void addHexGlyph(int cp, String data, int width) {
    ...
    for (int y = 0; y < 16; y++) {
      long row;
      try {
        row = Long.parseLong(data.substring(y * digits, (y + 1) * digits), 16);
      } catch (RuntimeException ex) {
        return;
      }
      for (int x = 0; x < width; x++)
        if ((row & (1L << (width - 1 - x))) != 0) {
    ...
    if (max >= min) {
      ...
      glyph.u0 = hexX / (float) ATLAS_SIZE;
      glyph.v0 = hexY / (float) ATLAS_SIZE;
      glyph.u1 = (hexX + width) / (float) ATLAS_SIZE;
      glyph.v1 = (hexY + 16) / (float) ATLAS_SIZE;
      glyph.width = width / 2F;
      glyph.height = 8F;
      glyph.advance = (max - min + 1) / 2F + 1F;
  ...
  public int drawString(String text, int x, int y, int color, boolean shadow) {
    if (text == null
        || ((glyphs.isEmpty() && spaces.isEmpty()) && !TooltipElements.containsTag(text)))
  ...
  private void drawGlyph(Glyph g, float x, float y, int color) {
    ResourceLocation texture =
        g.atlas && g.atlasPage < hexAtlasLocations.size()
            ? hexAtlasLocations.get(g.atlasPage)
            : g.texture;
    ...
    if (a == 0) a = 1;
    GL11.glColor4f(
        ((color >> 16) & 255) / 255F, ((color >> 8) & 255) / 255F, (color & 255) / 255F, a);
  ...
  public int getStringWidth(String text) {
    ...
    if (hasObfuscatedFormat(text) || isLegacyEncoded(text))
      return legacyRenderer.getStringWidth(text);
    float width = 0;
    for (int offset = 0; offset < text.length(); ) {
      if (text.charAt(offset) == '\u00a7' && offset + 1 < text.length()) {
        offset += 2;
  ...
  private void wrapParagraph(
      String paragraph, int width, String inheritedFormat, List<String> result) {
  ...
  private float advance(int cp) {
    ...
    return space != null
        ? space
        : glyph != null ? glyph.advance : super.getStringWidth(new String(Character.toChars(cp)));
  }
    ...
    for (int i = 0; i + 1 < text.length(); i++) {
      ...
      if ("0123456789abcdef".indexOf(code) >= 0) {
        ...
        styles.setLength(0);
          ...
          && styles.indexOf("\u00a7" + String.valueOf(code)) < 0) {
  ...
  private static int shadowColor(int color) {
    return (color & 0xFC000000) | ((color & 0xFCFCFC) >> 2);
  }
  ...
  private static int opaqueRight(BufferedImage image, int ox, int oy, int w, int h) {
    for (int x = w - 1; x >= 0; x--)
      for (int y = 0; y < h; y++) if ((image.getRGB(ox + x, oy + y) >>> 24) != 0) return x;
    return -1;
  }

  private static int[] codePoints(String s) {
    int[] out = new int[s.codePointCount(0, s.length())];
    for (int i = 0, o = 0; i < s.length(); o++) {
      out[o] = s.codePointAt(i);
      i += Character.charCount(out[o]);
    }
    return out;
  }
  ...
  private static ResourceLocation directLocation(String value) {
