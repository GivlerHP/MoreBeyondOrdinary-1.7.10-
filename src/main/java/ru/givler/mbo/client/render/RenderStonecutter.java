...
public class RenderStonecutter implements ISimpleBlockRenderingHandler {
  ...
  public RenderStonecutter(int id) {
    this.id = id;
  }

  @Override
  public boolean renderWorldBlock(
      IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks r) {
  ...
  public void renderInventoryBlock(Block block, int meta, int modelId, RenderBlocks r) {
    ...
    t.startDrawingQuads();
    t.setNormal(0, 0, -1);
    r.renderFaceZNeg(block, 0, .5625, 0, ((BlockStonecutter) block).saw);
    t.draw();
    t.startDrawingQuads();
    t.setNormal(0, 0, 1);
    r.renderFaceZPos(block, 0, .5625, 0, ((BlockStonecutter) block).saw);
    t.draw();
    GL11.glPopMatrix();
  }

  private void cube(
      Block b, int m, RenderBlocks r, double a, double c, double d, double e, double f, double g) {
    r.setRenderBounds(a, c, d, e, f, g);
    Tessellator t = Tessellator.instance;
    t.startDrawingQuads();
    t.setNormal(0, -1, 0);
    r.renderFaceYNeg(b, 0, 0, 0, b.getIcon(0, m));
    t.draw();
    t.startDrawingQuads();
    t.setNormal(0, 1, 0);
    r.renderFaceYPos(b, 0, 0, 0, b.getIcon(1, m));
    t.draw();
    t.startDrawingQuads();
    t.setNormal(0, 0, -1);
    r.renderFaceZNeg(b, 0, 0, 0, b.getIcon(2, m));
    t.draw();
    t.startDrawingQuads();
    t.setNormal(0, 0, 1);
    r.renderFaceZPos(b, 0, 0, 0, b.getIcon(3, m));
    t.draw();
    t.startDrawingQuads();
    t.setNormal(-1, 0, 0);
    r.renderFaceXNeg(b, 0, 0, 0, b.getIcon(4, m));
    t.draw();
    t.startDrawingQuads();
    t.setNormal(1, 0, 0);
    r.renderFaceXPos(b, 0, 0, 0, b.getIcon(5, m));
    t.draw();
  }
