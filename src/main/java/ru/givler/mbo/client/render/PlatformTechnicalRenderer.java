...
public final class PlatformTechnicalRenderer {
  ...
  public void render(RenderWorldLastEvent event) {
    ...
    if (mc.thePlayer == null || mc.theWorld == null || mc.renderViewEntity == null) return;
    ItemStack held = mc.thePlayer.getCurrentEquippedItem();
    if (held == null
        || !(held.getItem() instanceof ItemPlatformEditor)
        || !BuilderAccess.canEdit(mc.thePlayer)) return;
    double
        cx =
    ...
    for (Object value : mc.theWorld.loadedEntityList)
      if (value instanceof EntityMovingPlatform) {
        ...
        double x = p.prevPosX + (p.posX - p.prevPosX) * event.partialTicks - cx,
            y = p.prevPosY + (p.posY - p.prevPosY) * event.partialTicks - cy,
            z = p.prevPosZ + (p.posZ - p.prevPosZ) * event.partialTicks - cz;
        outline(
            x - .002D,
            y - .002D,
            z - .002D,
  ...
  private static void outline(double x, double y, double z, double X, double Y, double Z) {
    GL11.glPushAttrib(
  ...
  private static void edge(
      Tessellator t, double x, double y, double z, double X, double Y, double Z) {
    t.addVertex(x, y, z);
    t.addVertex(X, Y, Z);
  }
}
