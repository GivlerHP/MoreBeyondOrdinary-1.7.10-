...
public final class RenderRailBlock implements ISimpleBlockRenderingHandler {
  ...
  public boolean renderWorldBlock(
    ...
    for (Element element : model.elements)
      renderElement(t, model, element, choice.yRotation, x, y, z);
    return true;
  }
  ...
  private void renderElement(
      Tessellator t, Model model, Element e, int yRotation, int wx, int wy, int wz) {
  ...
  private void emit(
      ...
      int wz,
      double[]... vertices) {
    Face f = e.faces.get(side);
    if (f == null) return;
    String texture = model.textures.get(f.texture.substring(1));
    IIcon icon = icons.get(baseName(texture));
    if (icon == null) return;
  ...
  private static Model load(String name) {
    InputStream in =
        RenderRailBlock.class.getResourceAsStream("/assets/mbo/models/rails/" + name + ".json");
    ...
    try {
      ...
      for (Map.Entry<String, JsonElement> v : textures.entrySet())
        m.textures.put(v.getKey(), v.getValue().getAsString());
      for (JsonElement value : root.getAsJsonArray("elements")) {
        JsonObject o = value.getAsJsonObject();
        Element e = new Element();
        e.from = array(o.getAsJsonArray("from"));
        e.to = array(o.getAsJsonArray("to"));
        if (o.has("rotation")) {
      ...
      throw new RuntimeException("Could not load rail model " + name, ex);
    } finally {
      try {
        in.close();
  ...
  private static double[] array(JsonArray a) {
  ...
  private static final class Model {
    final Map<String, String> textures = new HashMap<String, String>();
  ...
  private static final class Element {
    double[] from, to;
    Rotation rotation;
    final Map<String, Face> faces = new HashMap<String, Face>();
  }
