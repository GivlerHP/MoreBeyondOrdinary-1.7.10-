...
public final class SmoothOpeningRenderer {
  ...
  private static final ThreadLocal<double[]> RESULT =
      new ThreadLocal<double[]>() {
        protected double[] initialValue() {
          return new double[3];
        }
      };
  private static final ThreadLocal<float[]> COLOR_RESULT =
  ...
  public static boolean begin(RenderBlocks renderer, Block block, int x, int y, int z) {
    ...
    Snapshot snapshot = snapshot(access, block, x, y, z);
  ...
  public static double[] transform(double x, double y, double z) {
    ...
    State s = c.state;
    float shown = s.shownAngle(c.now);
    float target = s.open ? 90F : 0F;
  ...
  public static float[] correctColor(float red, float green, float blue) {
    ...
    if (max > 0F && max < 0.8F) {
      ...
      out[0] = Math.min(1F, red * scale);
      out[1] = Math.min(1F, green * scale);
      out[2] = Math.min(1F, blue * scale);
    }
    return out;
  }
  ...
  public static int correctBrightness(int brightness) {
    ...
    int corrected =
        state.block.getMixedBrightnessForBlock(access, state.x, context.renderY, state.z);
  ...
  private static void rotateGate(double[] p, Context context, float delta) {
    ...
    if (alongX) {
      px = s.x + (second ? .875D : .125D);
      pz = s.z + .5;
    } else {
      px = s.x + .5;
      pz = s.z + (second ? .875D : .125D);
    }
  ...
  private static boolean isCarpenterFixedGatePost(RenderBlocks renderer, boolean alongX) {
    ...
    // the animated leaf.
    return Math.abs(crossCenter - .5D) < .02D
        && axisWidth <= .14D
        && crossWidth <= .14D
  ...
  public static boolean splitCarpenterGateCuboid(
      Object handler,
      net.minecraft.item.ItemStack cover,
      int x,
      int y,
      int z,
      double minX,
    ...
    Context context = ACTIVE.get();
    if (context == null
        || !isCarpenterGate(context.state.block)
        || context.state.open
        || !context.state.active(context.now)
        || Boolean.TRUE.equals(SPLITTING_CARPENTER_CUBOID.get())
        || minZ >= .5D
        || maxZ <= .5D) return false;
    try {
      if (carpenterRenderCuboid == null) {
        carpenterRenderCuboid =
            handler
                .getClass()
                .getSuperclass()
                .getDeclaredMethod(
      ...
      carpenterRenderCuboid.invoke(
          handler, cover, x, y, z, minX, minY, minZ, maxX, maxY, .5D, rotations);
      carpenterRenderCuboid.invoke(
          handler, cover, x, y, z, minX, minY, .5D, maxX, maxY, maxZ, rotations);
      return true;
  ...
  private static void rotateY(double[] p, double px, double pz, float degrees) {
  ...
  private static void rotateX(double[] p, double py, double pz, float degrees) {
    double a = Math.toRadians(degrees),
        c = Math.cos(a),
        s = Math.sin(a),
        y = p[1] - py,
  ...
  private static void rotateZ(double[] p, double px, double py, float degrees) {
    double a = Math.toRadians(degrees),
        c = Math.cos(a),
        s = Math.sin(a),
        x = p[0] - px,
  ...
  public void tick(TickEvent.ClientTickEvent e) {
    ...
    for (Map.Entry<Key, State> entry : STATES.entrySet()) {
      ...
      if (!s.active(now) && s.wasActive) {
        s.wasActive = false;
        w.markBlockRangeForRenderUpdate(s.x, s.y, s.z, s.x, s.y + (s.kind == DOOR ? 1 : 0), s.z);
      }
    }
  }
  ...
  public void interact(PlayerInteractEvent event) {
    ...
    if (state == null || state.block != block)
      STATES.put(
          key,
          new State(
              block, event.x, snapshot.baseY, event.z, snapshot.meta, open, now, snapshot.kind));
    else {
  ...
  private static final class Context {
    ...
    final long now;
    final int renderY;
    final boolean hideChunkCopy;

    Context(State s, RenderBlocks r, long n, int y, boolean h) {
      state = s;
      renderer = r;
      now = n;
      renderY = y;
  ...
  private static final class Key {
    ...
    public int hashCode() {
      return x * 7340033 ^ y ^ z * 19349663;
    }

    public boolean equals(Object o) {
      return o instanceof Key && ((Key) o).x == x && ((Key) o).y == y && ((Key) o).z == z;
    }
  }
  ...
  private static Snapshot snapshot(IBlockAccess access, Block block, int x, int y, int z) {
    ...
    if (block instanceof BlockDoor) {
      ...
      return new Snapshot(DOOR, by, ((BlockDoor) block).func_150012_g(access, x, by, z));
    }
    ...
    if (thaumcraftLoaded && ThaumcraftOpeningBridge.isArcaneDoor(block)) {
      int by = (raw & 8) != 0 ? y - 1 : y;
      return new Snapshot(
          DOOR, by, ThaumcraftOpeningBridge.fullDoorMetadata(block, access, x, by, z));
    }
  ...
  private static final class Snapshot {
    final int kind, baseY, meta;

    Snapshot(int k, int y, int m) {
      kind = k;
      baseY = y;
      meta = m;
    }
  ...
  private static final class State {
    ...
    State(Block b, int x, int y, int z, int m, boolean o, long n, int k) {
      ...
      meta = m;
      open = o;
      start = n - DURATION - RESTORE_GRACE;
      from = to = o ? 90 : 0;
      seen = n;
      kind = k;
    }
    ...
    boolean active(long n) {
      return n - start < DURATION;
    }

    boolean visibleDuringRestore(long n) {
      return n - start < DURATION + RESTORE_GRACE;
    }
