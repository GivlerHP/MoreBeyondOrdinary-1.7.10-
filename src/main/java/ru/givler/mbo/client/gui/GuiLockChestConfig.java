...
public class GuiLockChestConfig extends GuiContainer {
  ...
  public GuiLockChestConfig(InventoryPlayer player, TileEntityLockableChest chest) {
    super(new ContainerLockConfig(player, chest));
    this.chest = chest;
    xSize = 340;
    ySize = 276;
    difficulty = chest.getLockData().getDifficulty().ordinal();
    mode = chest.getRefillMode().ordinal();
    fill = chest.getFillSlots();
    cooldown = chest.getCooldownSeconds();
    interval = chest.getGradualIntervalSeconds();
    count = chest.getGradualCount();
    radius = chest.getLockData().getPlayerRadius();
    weighted = chest.isWeightedSelection();
    for (int i = 0; i < 14; i++) {
      weights[i] = chest.getLootWeight(i);
      guaranteed[i] = chest.isGuaranteed(i);
    }
  }
  ...
  public void initGui() {
    ...
    fillField = numberField(x, y + 81, 70, fill);
    cooldownField = numberField(x + 74, y + 81, 70, cooldown);
    intervalField = numberField(x, y + 111, 70, interval);
    countField = numberField(x + 74, y + 111, 70, count);
    ...
    group.setText(chest.getGroupId());
    group.setMaxStringLength(64);
  ...
  private String difficultyLabel() {
    return I18n.format(
        "mbo.lock.gui.difficulty.value",
        I18n.format(
            "mbo.lock.difficulty." + LockDifficulty.byOrdinal(difficulty).name().toLowerCase()));
  }

  private String modeLabel() {
    return I18n.format(
        "mbo.lock.gui.refill.value",
        I18n.format("mbo.lock.refill." + RefillMode.byOrdinal(mode).name().toLowerCase()));
  }

  private String selectionLabel() {
    return I18n.format(
        "mbo.lock.gui.selection.value",
        I18n.format(weighted ? "mbo.lock.selection.weighted" : "mbo.lock.selection.equal"));
  }

  private String guaranteeLabel() {
    return I18n.format(
        "mbo.lock.gui.guaranteed.short",
        I18n.format(guaranteed[selected] ? "mbo.lock.state.on" : "mbo.lock.state.off"));
  }
  ...
  protected void actionPerformed(GuiButton button) {
    if (button.id == 1) {
      ...
      button.displayString = difficultyLabel();
    } else if (button.id == 2) {
      mode = (mode + 1) % 3;
      button.displayString = modeLabel();
    } else if (button.id == 3) {
      weighted = !weighted;
      button.displayString = selectionLabel();
    } else if (button.id == 9) {
      guaranteed[selected] = !guaranteed[selected];
      button.displayString = guaranteeLabel();
    } else if (button.id == 11) {
      save();
      mc.displayGuiScreen(null);
    } else if (button.id == 12) {
      mc.displayGuiScreen(
          new GuiLockTemplates(chest.xCoord, chest.yCoord, chest.zCoord, buildTemplate()));
    }
  }
  ...
  private void save() {
    ...
    PacketManager.INSTANCE.sendToServer(
        new PacketSetLockDifficulty(chest.xCoord, chest.yCoord, chest.zCoord, difficulty));
    NBTTagCompound tag = new NBTTagCompound();
    tag.setInteger("RefillMode", mode);
    tag.setBoolean("Weighted", weighted);
    tag.setInteger("FillSlots", fill);
    tag.setInteger("Cooldown", cooldown);
    tag.setInteger("GradualInterval", interval);
    tag.setInteger("GradualCount", count);
    tag.setInteger("PlayerRadius", radius);
    tag.setString("GroupId", group.getText());
    tag.setIntArray("Weights", weights);
    for (int i = 0; i < 14; i++) tag.setBoolean("Guaranteed" + i, guaranteed[i]);
    PacketManager.INSTANCE.sendToServer(
        new PacketLockLootSettings(chest.xCoord, chest.yCoord, chest.zCoord, tag));
  }

  private void readFields() {
    fill = value(fillField, fill, 1, 27);
    cooldown = value(cooldownField, cooldown, 1, 31536000);
    interval = value(intervalField, interval, 1, 31536000);
    count = value(countField, count, 1, 27);
  ...
  private NBTTagCompound buildTemplate() {
    ...
    result.setInteger("Difficulty", difficulty);
    result.setInteger("RelockDelay", chest.getLockData().getRelockDelaySec());
    settings.setInteger("RefillMode", mode);
    settings.setBoolean("Weighted", weighted);
    settings.setInteger("FillSlots", fill);
    settings.setInteger("Cooldown", cooldown);
    settings.setInteger("GradualInterval", interval);
    settings.setInteger("GradualCount", count);
    settings.setInteger("PlayerRadius", radius);
    settings.setString("GroupId", group.getText());
    settings.setIntArray("Weights", weights);
    for (int i = 0; i < 14; i++) settings.setBoolean("Guaranteed" + i, guaranteed[i]);
    result.setTag("Settings", settings);
    NBTTagList items = new NBTTagList();
    ContainerLockConfig container = (ContainerLockConfig) inventorySlots;
    for (int i = 0; i < 14; i++) {
      ItemStack stack = container.getTemplate(i);
      if (stack != null) {
  ...
  private int value(GuiTextField field, int fallback, int min, int max) {
    try {
      return Math.max(min, Math.min(max, Integer.parseInt(field.getText())));
  ...
  protected void mouseClicked(int mx, int my, int button) {
    ...
    for (GuiTextField field : fields()) field.mouseClicked(mx, my, button);
    group.mouseClicked(mx, my, button);
    int sx = mx - (guiLeft + 17), sy = my - (guiTop + 32);
    if (sx >= 0 && sx < 126 && sy >= 0 && sy < 36) {
      weights[selected] = value(weightField, weights[selected], 0, 1000000);
      selected = (sy / 18) * 7 + sx / 18;
      weightField.setText(Integer.toString(weights[selected]));
      for (Object object : buttonList) {
        GuiButton b = (GuiButton) object;
        if (b.id == 9) b.displayString = guaranteeLabel();
      }
    }
  }
  ...
  private GuiTextField[] fields() {
    return new GuiTextField[] {
      fillField, cooldownField, intervalField, countField, radiusField, weightField
    };
  }
    ...
    if (Character.isDigit(ch)
        ...
        || (isCtrlKeyDown()
            && (key == Keyboard.KEY_A
                || key == Keyboard.KEY_C
                || key == Keyboard.KEY_V
                || key == Keyboard.KEY_X))) field.textboxKeyTyped(ch, key);
    return true;
  }
  ...
  protected void drawGuiContainerBackgroundLayer(float p, int mx, int my) {
    ...
    for (int i = 0; i < 14; i++) {
      int x = guiLeft + 16 + (i % 7) * 18, y = guiTop + 31 + (i / 7) * 18;
      drawRect(x, y, x + 18, y + 18, i == selected ? 0xffb08030 : 0xff555555);
    }
